...
class IPTVPlayer:
    def __init__(self):
        ...
        self.playing = False
        self.mpv_player = mpv.MPV()  # Inicjalizacja odtwarzacza mpv
    ...
    def toggle_play(self, widget):
        if not self.playing:
            ...
            if uri:
                self.mpv_player.play(uri)
                self.playing = True
                self.play_button.set_label("Pause")
        else:
            self.mpv_player.command('osd-msg', 'cycle pause')  # Zmiana stanu pauzy
            self.playing = not self.playing
            self.play_button.set_label("Play" if self.playing else "Pause")
