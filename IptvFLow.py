...
import re
import os
from gi.repository import Gtk, GdkPixbuf, Gdk, GObject, GLib, Gst
import notify2
import time
import configparser
...
class IPTVPlayer(Gtk.Window):
    def __init__(self):
        print("Initializing IPTVPlayer...")
        # ... (rest of your initialization code)

        super().__init__(title="IPTV Player")
        ...
        self.box.set_size_request(800, 700)  # Set a fixed height of 300 pixels (adjust as needed)
        self.box.set_vexpand(False)
        self.box.set_hexpand(False)
        self.box.set_valign(Gtk.Align.START)  # Align to the top
        ...
        # Additional initialization for CoverFlow
        self.scrolled_window = Gtk.ScrolledWindow()
        self.scrolled_window.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
        self.scrolled_window.set_size_request(800, 100)
        # Use Gtk.Grid instead of Gtk.HBox
        self.grid = Gtk.Grid()
        self.grid.set_row_spacing(1)
        self.grid.set_column_spacing(1)
        self.scrolled_window.add(self.grid)

        playlist_url = "http://192.168.1.133:9981/playlist/channels.m3u"
        playlist_content = self.fetch_playlist(playlist_url)
        image_info = self.extract_image_info(playlist_content)

        self.pixbufs = [self.load_image_from_url(info['url'], info['name'], info['stream_url']) for info in image_info]
        self.current_index = 0
        self.previous_stream_pid = None

        self.create_images()

        self.connect("key-press-event", self.on_key_press)
        print("IPTVPlayer initialization complete.")

    def fetch_playlist(self, url):
        response = requests.get(url)
        return response.text if response.status_code == 200 else ""

    def extract_image_info(self, playlist_content):
        image_info = []
        regex_name = re.compile(r'#EXTINF:[^,]*,(.+)\s*$', re.MULTILINE)
        names = [match.group(1) for match in regex_name.finditer(playlist_content)]

        regex_info = re.compile(r'logo="(.+)",\s*(.+)\s*\n(.+)$', re.MULTILINE)
        matches = regex_info.finditer(playlist_content)

        for match, name in zip(matches, names):
            logo_url = match.group(1)
            stream_url = match.group(3)
            image_info.append({'url': logo_url, 'name': name, 'stream_url': stream_url})

        return image_info

    def load_image_from_url(self, url, name, stream_url):
        response = requests.get(url)
        if response.status_code == 200:
            ...
            if pixbuf is not None:
                scaled_pixbuf = pixbuf.scale_simple(
                    80, 40, GdkPixbuf.InterpType.BILINEAR
                )

                image_with_label = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=5)
                image = Gtk.Image.new_from_pixbuf(scaled_pixbuf)
                label = Gtk.Label(label=name)
                image_with_label.pack_start(image, True, True, 0)
                image_with_label.pack_start(label, False, False, 0)

                # Save the stream URL
                image_with_label.stream_url = stream_url

                return image_with_label
            else:
                return GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, True, 8, 1, 1).fill(0xFFFFFFFF)
        else:
            return GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, True, 8, 1, 1).fill(0xFFFFFFFF)
    ...
    def play_stream(self):
        if 0 <= self.current_index < len(self.pixbufs) and self.pixbufs[self.current_index] is not None:
            stream_url = self.pixbufs[self.current_index].stream_url
            if stream_url:
                print(f"Playing stream: {stream_url}")

                # Stop the previous playback
                self.stop_previous_stream()

                # Start the new playback
                process = subprocess.Popen(["gst-launch-1.0", "playbin", "uri=" + stream_url])
                self.previous_stream_pid = process.pid
                self.update_images()

    def stop_previous_stream(self):
        # Check if there is a previous GStreamer process
        if self.previous_stream_pid is not None:
            try:
                # Terminate the previous GStreamer process
                subprocess.run(["kill", str(self.previous_stream_pid)], check=True)
            except subprocess.CalledProcessError:
                print(f"Error stopping the previous GStreamer process (PID: {self.previous_stream_pid})")
    ...
    def show_next_image(self):
        if self.current_index < len(self.pixbufs) - 1:
            self.current_index += 1
            self.update_images()
    ...
    def create_images(self):
        middle_index = self.current_index + 0

        for i in range(middle_index - 0, middle_index + 1):
            if 0 <= i < len(self.pixbufs) and self.pixbufs[i] is not None:
                if i == middle_index:
                    scaled_pixbuf = self.pixbufs[i].get_children()[0].get_pixbuf().scale_simple(
                        int(self.pixbufs[i].get_children()[0].get_pixbuf().get_width() * 2),
                        int(self.pixbufs[i].get_children()[0].get_pixbuf().get_height() * 2),
                        GdkPixbuf.InterpType.BILINEAR
                    )
                    image = Gtk.Image.new_from_pixbuf(scaled_pixbuf)
                else:
                    image = Gtk.Image.new_from_pixbuf(self.pixbufs[i].get_children()[0].get_pixbuf())
            else:
                image = Gtk.Image()
        ...
        if 0 <= self.current_index < len(self.pixbufs) and self.pixbufs[self.current_index] is not None:
            self.scrolled_window.get_hadjustment().set_value(self.pixbufs[self.current_index].get_children()[0].get_pixbuf().get_width() * (middle_index - 4))
        self.show_all()
    ...
    def on_filechooser_button_clicked(self, widget):
        dialog = Gtk.FileChooserDialog(
            title="Select Playlist", parent=self, action=Gtk.FileChooserAction.OPEN
        )
